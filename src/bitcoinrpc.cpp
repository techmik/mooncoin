...
// Copyright (c) 2013 The Mooncoin Developers
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Mooncoin server has now stopped running!";
}
...
// Mooncoin: Return average network hashes per second based on last number of blocks.
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid Mooncoin address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid Mooncoin address:")+s.name_);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Mooncoin server stopping, restart to run with encrypted wallet";
}
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Mooncoin is not connected!!!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Mooncoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        result.push_back(Pair("target",   HexStr(BEGIN(hashTarget), END(hashTarget))));
        result.push_back(Pair("algorithm", "scrypt:1024,1,1"));  // Mooncoin: specify that we should use the scrypt algorithm
        return result;
    }
    else
    {
...
//
// JSON-RPC protocol.  Mooncoin speaks version 1.0 for maximum compatibility,
...
//    ip::tcp::endpoint endpoint(bindAddress, GetArg("-rpcport", 44663));
...
//    if (!d.connect(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "44663")))
