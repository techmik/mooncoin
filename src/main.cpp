...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	{
		nSubsidy = 2519841 * COIN; // for one week, the cost of the Apollo program will be paid back -- 25.4 bn lunas
	}
...
static const int64 nTargetSpacing = 90; // Mooncoin: 1 minute, 30 seconds
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal mooncoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Mooncoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Mooncoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
